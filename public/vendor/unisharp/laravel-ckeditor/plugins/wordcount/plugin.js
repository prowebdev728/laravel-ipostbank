    ...
    init: function (editor) {
       ...
       function counterId(editorInstance) {
        ...
        function countParagraphs(text) {
            var outresult = (text.replace(/&nbsp;/g, " ").replace(/(<([^>]+)>)/ig, "").replace(/^\s*$[\n\r]{1,}/gm, "++").split("++").length);
            return outresult;
        }
        ...
        function updateCounter(editorInstance) {
            ...
            $('[data-article-char]').val(charCount).change();
            $('[data-article-word]').val(wordCount).change();
            $('[data-article-para]').val(paragraphs).change();
