...
<div class="callout callout-danger">
    <h4>Temporary Permission Denied</h4>
    <p>This article is under the admin review. you are not able to edit right now. When your article approved we will let you know.</p>
</div>
@include('layouts.footer')
@elseif($article->status === "Used" && !$user->is_admin)
    <div class="callout callout-info">
        <p>You are no long able to edit this article. Because article has been sold.</p>
    </div>
    @include('layouts.footer')
@else
<br>
...
<section class="content">
    <div class="row">
        <form action="/article/updater/{{$article->article_id}}" method="post">
            <div class="col-md-9 ">
                {{csrf_field()}}
                <div class="box box-info editor-box">
                    ...
                    <div class="box-body pad">
                        <div class="form-group article-title {{ $errors->has('title') ? ' has-error' : '' }}">
                            ...
                            <input type="text" name="title" value="@if($errors->count()){{old('title')}}@else{{$article->title}}@endif" data-artile-title class="form-control" placeholder="Title">
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('summary') ? ' has-error' : '' }}">
                            <label>Summary</label>
                            <textarea class="form-control"  data-artile-sum  name="summary" rows="3" placeholder="You article Summary what you are going to represent.!" >@if($errors->count()){{old('summary')}}@else{{$article->summary}}@endif</textarea>
                        ...
                        <div class="form-group {{ $errors->has('composer') ? ' has-error' : '' }}">
                            <label>Compose</label>
                            <textarea id="editor1" class="hide" name="composer" rows="10" cols="80">@if($errors->count()){{old('composer')}}@else {{$article->text}} @endif
                            </textarea>
                            ...
                            @foreach($cat_array as $cat)
                                @if(old('cat'))
                                       @if(in_array($cat->name, old('cat')))
                                        ...
                                        @endif
                                @else
                                    <?php $cat_data_array = explode(',',$article->cat); ?>
                                         @if(in_array($cat->name, $cat_data_array))
                                            <?php $check = "checked"; ?>
                                         @else
                                            <?php $check = ""; ?>
                                         @endif
                        ...
                        <div class="form-group article-title col-md-12">
                            <select class="form-control" name="lang">
                                ...
                                @foreach($lang_array as $lang)
                                    ...
                                    @else
                                        <option value="{{$lang}}"  @if($lang === $article->lang)selected @endif >{{$lang}}</option>
                                    @endif
                        ...
                        <div class="modal-body">
                            <div class="box box-widget widget-user">
                                ...
                                <div class="widget-user-header bg-aqua-active">
                                    <h3 class="widget-user-username" data-modal-artile-title>{{$article->title}}</h3>
                                    <h5 class="widget-user-desc" data-modal-artile-sum>{{$article->summary}}</h5>
                                </div>
